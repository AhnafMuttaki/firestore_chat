...
export default {
  ...
  async clearUnreadMsg(context, payload) {
    // Clears unread message
    let query = firebase
      .collection("unread_records")
      .where("conv_id", "==", payload.conv_id)
  ...
  async sendMsg(context, payload) {
    // Sends messages by adding new message in message collection
    ...
    msgCollection
      ...
      .then(function(docRef) {
        ...
        // If message sent is successful then adds unread record for each user in that conversation except the sender
